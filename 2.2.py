...
obj = m.addMVar(1,vtype=GRB.CONTINUOUS)
...
#minimiser max i∈I d(i, f(i)) revient a minimiser obj tq obj >= d(i, f(i)) pour chaque i
# Pour chaque ville i : z >= d(i,f(i))
for i in range(n):
    d_ifi = 0
    for j in range(n):
        d_ifi += d_ij[i][j] * x[i,j]
    m.addConstr(obj >= d_ifi, "c%d" % (2*n + n*n + 1 + i*n + j))
